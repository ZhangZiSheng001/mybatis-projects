...
**将`RowBounds`作为 Mapper 方法的入参，可以支持自动分页功能，但是，这种方式存在一个很大缺点，就是 Mybatis 会将所有结果查放入本地内存再进行分页，而不是查的时候嵌入分页参数。所以，这个分页入参，建议还是不要使用了。**
...
这里我就不搞那么复杂了，就只看非嵌套结果的情况。进入`DefaultResultSetHandler.handleRowValuesForSimpleResultMap(ResultSetWrapper, ResultMap, ResultHandler<?>, RowBounds, ResultMapping)`。在这个方法中可以看到，使用`RowBounds`进行分页时，Mybatis 会查出所有数据到内存中，然后再分页，所以，不建议使用。
